...
UBTService_OrientToTargetActor::UBTService_OrientToTargetActor()
{
    ...
    InTargetActorKey.AddObjectFilter(this, GET_MEMBER_NAME_CHECKED(UBTService_OrientToTargetActor, InTargetActorKey),
                                     AActor::StaticClass());
}
...
{
    const auto KeyName = InTargetActorKey.SelectedKeyName.ToString();
    return FString::Printf(TEXT("Orient rotation to %s Key %s"), *KeyName, *GetStaticTickIntervalDescription());
}
...
void UBTService_OrientToTargetActor::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
    ...
    if (TargetActor && OwningPawn)
    {
        ...
        const FRotator TargetRotation =
            FMath::RInterpTo(OwningPawn->GetActorRotation(), LookAtRotation, DeltaSeconds, RotationInterpSpeed);
